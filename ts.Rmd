...
$$Y_t - Y{t-1} = \epsilon_t \rightarrow diff(Y) \ is \ WN$$
## Simulate the random walk model with $\mu$=0
...
plot(random_walk, main="Random Walk with mu = 0")
```
...
if $\mu=0$ and slop $\phi=1$ then $$Y_t = Y_{t-1} + \epsilon_t$$ which is $$Today = Yesterday + Noise$$ which is a Random Walk (RW) process and $Y_t$ is not stationary in this case.
...
Recall the AR model about to fit and the displaye equation:
...
```{r}
predict(AR_fit, n.ahead = 10)$pred[1:10]
```
